...
現在の既定 runtime は 1 ターンあたり `5` 回の LLM 呼び出しです。
`AppraisalNode`、`ConflictEngineNode`、`ExpressionRealizerNode`、`FidelityGateNode`、`MemoryInterpreterNode` がターンを組み立て、その後に `MemoryCommitNode` が状態更新と永続記憶への保存を行います。
...
- `user_id x persona_name` 単位で分離された Markdown ベース永続記憶
- persona の固定自己認識名 `identity.self_name`
...
| `memory/` | frontmatter 付き Markdown による永続記憶 |
...
各 persona には `identity.self_name` と `identity.display_name` があり、会話のたびに名乗らせるためではなく、「AI が自分をどう認識しているか」を固定するために使います。
...
- 既定の turn pipeline は `session_bootstrap -> appraisal -> conflict_engine -> expression_realizer -> fidelity_gate -> memory_interpreter -> memory_commit` です。
- ペルソナ設定は、`identity`、psychodynamics、relational profile、defense organization、ego organization、safety boundary を中心にした v2 schema です。
- 永続記憶は `data/memory/<user_id>/<persona_name>/` 配下の Markdown cards / digests で管理します。
